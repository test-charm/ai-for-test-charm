...
public class DALMockServer {
    ...
    public void mock(Map<String, List<ResponseBuilder>> expectationResponses) {
        ...
        mockServerClient.when(request()).respond(httpRequest -> {
            ResponseBuilder matchedResponse = null;
            ...
            synchronized (lock) {
                    ...
                    if (responseBuilders.size() == 1) {
                        ...
                        try {
                            ...
                            if (currentTime >= responseBuilders.size()) {
                                ...
                                matchedResponse = responseBuilders.get(currentTime);
                                break;
                            }
                        } catch (SyntaxException e) {
                            errorMessage = e.getMessage() + "\n\n" + e.show(expectation);
                ...
                if (matchedResponse == null && errorMessage == null) {
                    errorMessage = buildMessage(httpRequest, unexpectations);
                }
            }
            if (errorMessage != null) {
                return matchedResponse == null
                        ? HttpResponse.response(errorMessage).withStatusCode(404)
                        : HttpResponse.response().withStatusCode(500).withBody(errorMessage);
            }
            if (matchedResponse.delayResponse > 0) delay(matchedResponse);
            HttpResponse httpResponse = HttpResponse.response().withStatusCode(matchedResponse.code)
                    .withContentType(MediaType.APPLICATION_JSON);
            matchedResponse.giveBody(httpResponse);
            matchedResponse.buildHeaders().forEach(httpResponse::withHeader);
            return httpResponse;
        });
    }
